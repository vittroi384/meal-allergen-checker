...
3. 시트 메뉴 **[급식 알레르기] → 웹앱 열기** 에서도 링크를 볼 수 있습니다.
...
| 7 | 새 계정 | 확장 프로그램 → Apps Script → **배포 → 새 배포 → 유형: 웹 앱** → 실행 사용자 "나", 액세스 "모든 사용자" → 배포 → **새 URL** 확보. (로컬 clasp 를 계속 쓰려면 새 계정으로 `clasp login` 후 `clasp clone-script <새 scriptId> --rootDir src`, `.deployment-id` 삭제 후 `npm run deploy`) |
